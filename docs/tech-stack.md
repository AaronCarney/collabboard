...
| API layer          | Direct Supabase + Next.js API routes | —                | Hot path stays fast (no HTTP round-trip); API routes for AI + sharing (D004)                                                        |
| Authentication     | Clerk                                | 6.x              | Best LLM-agent DX; typed SDK; webhooks for user sync                                                                                |
| Real-time          | Supabase Realtime Broadcast          | —                | Working, deployed, tested; optimistic updates + broadcast meet <100ms (D003)                                                        |
| Database           | Supabase (Postgres)                  | —                | RLS, real-time, storage — all first-party; free tier sufficient for MVP                                                             |
| DB types           | supabase gen types                   | —                | Generated TypeScript types from Postgres schema; no ORM overhead                                                                    |
...
| AI features        | TBD — pending Phase 4A eval          | —                | Structured outputs via Vercel AI SDK tool calling; provider selected by evaluation (see docs/plans/implementation-plan.md Phase 4A) |
...
| Hosting (realtime) | Supabase (included)                  | —                | Realtime broadcast bundled with Supabase; no separate hosting needed                                                                |
...
| tRPC                       | Adds HTTP round-trip to hot path (object mutations), violating <100ms sync target. Type safety achieved via `supabase gen types` + Zod. 24-36hr migration cost exceeds sprint budget (D004)               |
| Prisma                     | Not needed — Supabase PostgREST provides typed queries via generated types. Prisma adds ORM overhead, client regeneration on schema changes, and a dependency on Node.js runtime (incompatible with edge) |
| Cloudflare Durable Objects | Working Supabase Realtime meets all performance targets. DO migration would cost 2-3 days for marginal benefit at current scale (5+ users, not 300). Deferred to post-submission (D003)                   |

## Conflict Resolution Strategy

All object mutations use **Last-Write-Wins (LWW) with per-object version numbers** (D001):

1. Each object has a single `version` integer (starts at 1, increments on any mutation)
2. Every mutation sends the full object state (not deltas) with the incremented version
3. A mutation is applied only if `incoming.version >= existing.version`
4. Conflicts are resolved client-side via optimistic updates; Supabase Postgres is the persistence authority

**Why per-object (not per-property):** Discrete whiteboard objects (sticky notes, shapes, frames) are rarely edited on different properties simultaneously by different users. The edge case (User A drags while User B recolors the same object) is low-stakes — the last write wins and the user can redo their action. Per-property versioning adds payload size and merge complexity for a near-zero benefit at this scale. This matches Excalidraw's production approach.

**Exception — Rich text:** If rich text editing is added later, sticky note text content would use Yjs CRDT via TipTap editor. This is the only scenario where CRDT would be used.
